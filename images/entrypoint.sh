#!/usr/bin/env bash
# Entrypoint for the corral GitHub Actions runner images.
#
...
#            GITHUB_URL, RUNNER_AUTO_LABELS (default true: add the image flavor
#            and OS as labels), RUNNER_OS_RELEASE (os-release path, for tests)
set -euo pipefail
...
# merge_labels "a,b" "b, c" -> "a,b,c": order kept, blanks and duplicates dropped.
merge_labels() {
    local out="" item
    # shellcheck disable=SC2046  # splitting on commas/spaces is the point
    for item in $(printf '%s' "$*" | tr ',' ' '); do
        case ",$out," in *",$item,"*) ;; *) out="${out:+$out,}$item" ;; esac
    done
    printf '%s' "$out"
}

# auto_labels prints the labels that describe this image: the flavor baked
# into the image (RUNNER_FLAVOR) and the OS from /etc/os-release, e.g.
# "minideb,debian,debian-13". RUNNER_AUTO_LABELS=false disables it.
auto_labels() {
    [ "${RUNNER_AUTO_LABELS:-true}" = "true" ] || return 0
    local file="${RUNNER_OS_RELEASE:-/etc/os-release}" id="" version=""
    if [ -r "$file" ]; then
        # shellcheck disable=SC1090  # os-release is a plain KEY=value file
        id=$(. "$file" >/dev/null 2>&1; printf '%s' "${ID:-}")
        # shellcheck disable=SC1090
        version=$(. "$file" >/dev/null 2>&1; printf '%s' "${VERSION_ID:-}")
    fi
    merge_labels "${RUNNER_FLAVOR:-}" "$id" "${id:+$id${version:+-$version}}"
}

[ -n "${RUNNER_REGISTER_TO:-}" ] || die "RUNNER_REGISTER_TO is required: an organization (my-org) or owner/repo (my-org/my-repo)"
...
if [ -f .runner ]; then
    ...
    labels=$(merge_labels "${RUNNER_LABELS:-}" "$(auto_labels)")
    if [ -n "$labels" ]; then config_args+=(--labels "$labels"); fi
...
# The runner bundles several node runtimes: nodeNN (glibc, runs on this image)
# and nodeNN_alpine (musl, mounted into alpine job containers; this host cannot
# execute it). Pick the newest glibc build.
NODE_BIN=""
newest=0
for n in externals/node*/bin/node; do
    v=${n#externals/node}; v=${v%%/*}
    case "$v" in ''|*[!0-9]*) continue ;; esac
    if [ "$v" -gt "$newest" ]; then newest=$v; NODE_BIN=$n; fi
done
