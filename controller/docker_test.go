...
func TestListContainers_FiltersByLabelAndParsesFields(t *testing.T) {
	fd := newFakeDocker(t, func(w http.ResponseWriter, r *http.Request, _ []byte) bool {
		...
		_, _ = io.WriteString(w, `[
		  {"Id":"c1","Names":["/corral-aaaa1111"],"State":"running","Status":"Up 5 minutes (healthy)","Image":"img:latest","ImageID":"sha256:111","Created":1700000000,
		   "Labels":{"corral.managed":"true","corral.name":"corral-aaaa1111"}},
		  {"Id":"c2","Names":["/corral-bbbb2222"],"State":"exited","Status":"Exited (1) 2 minutes ago","Image":"img:latest","ImageID":"sha256:111","Created":1700000100,
		   "Labels":{"corral.managed":"true"}}
		]`)
		return true
	})
	list, err := fd.client(t).ListContainers(context.Background(), "corral.managed=true")
	if err != nil {
	...
	if !strings.Contains(req.Query, "corral.managed") {
	...
	c := list[0]
	if c.ID != "c1" || c.Name != "corral-aaaa1111" || c.State != "running" || c.ImageID != "sha256:111" {
		t.Errorf("parsed container: %+v", c)
	}
	if c.Health != "healthy" {
		t.Errorf("health from status text: got %q", c.Health)
	}
	if c.Labels["corral.name"] != "corral-aaaa1111" {
...
func TestInspectContainer_ReadsHealthAndRestartCount(t *testing.T) {
	fd := newFakeDocker(t, func(w http.ResponseWriter, r *http.Request, _ []byte) bool {
		...
		_, _ = io.WriteString(w, `{"Id":"c1","Name":"/corral-aaaa1111","Image":"sha256:111","RestartCount":3,"Created":"2026-08-24T10:00:00.000000000Z",
		  "State":{"Status":"running","Running":true,"ExitCode":0,"Health":{"Status":"unhealthy"}},
		  "Config":{"Image":"img:latest","Labels":{"corral.managed":"true"}}}`)
		return true
	})
	...
	if c.Name != "corral-aaaa1111" || c.State != "running" || c.Health != "unhealthy" || c.RestartCount != 3 || c.ImageID != "sha256:111" {
...
func TestCreateAndStartContainer_SendsExpectedSpec(t *testing.T) {
	...
	c := fd.client(t)
	spec := ContainerSpec{
		Name:          "corral-aaaa1111",
		Image:         "ghcr.io/oeasenet/corral/runner:latest",
		Hostname:      "corral-aaaa1111",
		Env:           []string{"RUNNER_TOKEN=abc", "RUNNER_NAME=corral-aaaa1111"},
		Labels:        map[string]string{"corral.managed": "true"},
	...
	req := fd.last()
	if !strings.Contains(req.Query, "name=corral-aaaa1111") {
	...
	if body.Labels["corral.managed"] != "true" {
...
func TestPullImage_SendsRegistryAuthAndDetectsStreamErrors(t *testing.T) {
	...
	fd := newFakeDocker(t, func(w http.ResponseWriter, r *http.Request, _ []byte) bool {
		if r.Method == http.MethodPost && r.URL.Path == "/v1.44/images/create" {
			_, _ = io.WriteString(w, `{"status":"Pulling from oeasenet/corral/runner"}`+"\n")
			if fail {
	...
	if err := c.PullImage(context.Background(), "ghcr.io/oeasenet/corral/runner:latest", auth); err != nil {
		t.Fatalf("PullImage: %v", err)
	}
	req := fd.last()
	if !strings.Contains(req.Query, "fromImage=ghcr.io%2Foeasenet%2Fcorral%2Frunner") || !strings.Contains(req.Query, "tag=latest") {
	...
	fail = true
	if err := c.PullImage(context.Background(), "ghcr.io/oeasenet/corral/runner:latest", nil); err == nil || !strings.Contains(err.Error(), "manifest unknown") {
...
func TestInspectImage_ReturnsIDAndLabelsOrZeroWhenMissing(t *testing.T) {
	fd := newFakeDocker(t, func(w http.ResponseWriter, r *http.Request, _ []byte) bool {
		switch r.URL.Path {
		case "/v1.44/images/img:present/json":
			_, _ = io.WriteString(w, `{"Id":"sha256:abc","Config":{"Labels":{"corral.flavor":"debian","corral.runner-version":"2.336.0"}}}`)
			return true
	...
	c := fd.client(t)
	info, err := c.InspectImage(context.Background(), "img:present")
	if err != nil || info.ID != "sha256:abc" || info.Labels["corral.flavor"] != "debian" || info.Labels["corral.runner-version"] != "2.336.0" {
		t.Errorf("present: %+v %v", info, err)
	}
	if info, err := c.InspectImage(context.Background(), "img:missing"); err != nil || info.ID != "" || info.Labels != nil {
		t.Errorf("missing must be zero value without error: %+v %v", info, err)
	}
}
