...
func TestParseTarget(t *testing.T) {
	org, err := ParseTarget("example-org")
	if err != nil || org.Owner != "example-org" || org.Repo != "" {
		t.Errorf("org: %+v %v", org, err)
	}
	if org.RunnersPath() != "/orgs/example-org/actions/runners" || org.URL("https://github.com") != "https://github.com/example-org" {
		t.Errorf("org paths: %s %s", org.RunnersPath(), org.URL("https://github.com"))
	}
	repo, err := ParseTarget("example-org/platform")
	if err != nil || repo.Owner != "example-org" || repo.Repo != "platform" {
		t.Errorf("repo: %+v %v", repo, err)
	}
	if repo.RunnersPath() != "/repos/example-org/platform/actions/runners" || repo.URL("https://github.com/") != "https://github.com/example-org/platform" {
...
func TestRegistrationToken_OrgAndRepo(t *testing.T) {
	...
	org := NewGitHubClient(gh.URL, "ghp_pat", Target{Owner: "example-org"})
	tok, err := org.RegistrationToken(context.Background())
	if err != nil || tok != "AREGTOKEN" {
		t.Fatalf("org token: %q %v", tok, err)
	}
	if gh.paths[0] != "/orgs/example-org/actions/runners/registration-token?" {
	...
	repo := NewGitHubClient(gh.URL+"/", "ghp_pat", Target{Owner: "example-org", Repo: "platform"})
	if _, err := repo.RegistrationToken(context.Background()); err != nil {
		t.Fatalf("repo token: %v", err)
	}
	if gh.paths[1] != "/repos/example-org/platform/actions/runners/registration-token?" {
...
func TestRegistrationToken_SurfacesGitHubErrorsWithoutPAT(t *testing.T) {
	...
	_, err := NewGitHubClient(gh.URL, "ghp_secret", Target{Owner: "example-org"}).RegistrationToken(context.Background())
...
func TestListRunners_Paginates(t *testing.T) {
	gh := newFakeGitHubAPI(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case "", "1":
			_, _ = io.WriteString(w, `{"total_count":3,"runners":[
			  {"id":1,"name":"corral-aaaa1111","os":"Linux","status":"online","busy":true,"labels":[{"name":"self-hosted"},{"name":"docker"}]},
			  {"id":2,"name":"corral-bbbb2222","os":"Linux","status":"offline","busy":false,"labels":[]}]}`)
		case "2":
	...
	runners, err := NewGitHubClient(gh.URL, "ghp_pat", Target{Owner: "example-org"}).ListRunners(context.Background())
	if err != nil {
	...
	if runners[0].ID != 1 || runners[0].Name != "corral-aaaa1111" || runners[0].Status != "online" || !runners[0].Busy {
...
func TestDeleteRunner_TreatsMissingAsDeleted(t *testing.T) {
	gh := newFakeGitHubAPI(t, func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/orgs/example-org/actions/runners/1":
			w.WriteHeader(http.StatusNoContent)
		case "/orgs/example-org/actions/runners/2":
			http.Error(w, `{"message":"Not Found"}`, http.StatusNotFound)
		default:
			http.Error(w, `{"message":"Resource not accessible by personal access token"}`, http.StatusForbidden)
		}
	})
	c := NewGitHubClient(gh.URL, "ghp_pat", Target{Owner: "example-org"})
...
func TestLatestRunnerVersion_StripsVPrefix(t *testing.T) {
	gh := newFakeGitHubAPI(t, func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/repos/actions/runner/releases/latest" {
			_, _ = io.WriteString(w, `{"tag_name":"v2.336.0","name":"v2.336.0"}`)
			return
		}
		http.NotFound(w, r)
	})
	v, err := NewGitHubClient(gh.URL, "ghp_pat", Target{Owner: "example-org"}).LatestRunnerVersion(context.Background())
	if err != nil || v != "2.336.0" {
