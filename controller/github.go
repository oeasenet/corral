...
func (g *GitHubClient) do(ctx context.Context, method, path string) (*http.Response, error) {
	...
	req.Header.Set("User-Agent", "corral/"+version)
...
// LatestRunnerVersion returns the newest actions/runner release (e.g. "2.336.0").
func (g *GitHubClient) LatestRunnerVersion(ctx context.Context) (string, error) {
	resp, err := g.do(ctx, http.MethodGet, "/repos/actions/runner/releases/latest")
	if err != nil {
		return "", err
	}
	var body struct {
	...
	if err := decodeJSON(resp, &body); err != nil || body.TagName == "" {
		return "", errors.New("github api returned an unexpected release response")
	}
